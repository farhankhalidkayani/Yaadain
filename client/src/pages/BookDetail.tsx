...
import { getBook, updateBook, getUserMemories, addMemoryToBook, removeMemoryFromBook, uploadImage } from '@/lib/firebase';
import { useToast } from '@/hooks/use-toast';
import { ArrowLeft, BookOpen, Edit, ShoppingCart, Check, Plus, Image as ImageIcon, Trash2 } from 'lucide-react';
...
const BookDetail = () => {
  ...
  useEffect(() => {
    const fetchBookDetails = async () => {
      ...
      try {
        ...
        // Filter memories already in the book
        const bookMemoryIds = fetchedBook.memories?.map((memory: any) => memory.id) || [];
        const memoriesInBook = memories.filter((memory) => bookMemoryIds.includes(memory.id));
        const memoriesNotInBook = memories.filter((memory) => !bookMemoryIds.includes(memory.id));
  ...
  const handleAddMemoriesToBook = async () => {
    ...
    try {
      // Add each selected memory to the book
      ...
      setAvailableMemories(availableMemories.filter((memory) => !selectedMemoryIds.includes(memory.id)));
      setSelectedMemoryIds([]);
      setIsAddMemoryDialogOpen(false);
      
      toast({
        title: 'Success',
        description: `Added ${addedMemories.length} ${addedMemories.length === 1 ? 'memory' : 'memories'} to your book`,
      });
    } catch (error) {
  ...
  const handleCompleteBook = async () => {
    ...
    try {
      ...
      toast({
        title: 'Success',
        description: 'Book marked as complete! You can now order a printed copy.',
      });
    } catch (error) {
      ...
      <div className="min-h-screen flex flex-col bg-[#FDFBF8] bg-[url('https://www.transparenttextures.com/patterns/paper.png')] bg-fixed">
        ...
        <div className="flex-grow flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-neutral-500 mb-4">The memory book you're looking for doesn't exist or was deleted.</p>
            <Button asChild>
    ...
    <div className="min-h-screen flex flex-col bg-[#FDFBF8] bg-[url('https://www.transparenttextures.com/patterns/paper.png')] bg-fixed">
      ...
      <main className="flex-grow">
        <div className="container mx-auto px-4 py-6">
          <div className="mb-6">
            <Button variant="ghost" onClick={() => navigate('/memory-books')} className="mb-4">
            ...
            <div className="flex flex-wrap gap-4 text-sm text-neutral-500 mb-6">
              ...
              <div>
                {bookMemories.length} {bookMemories.length === 1 ? 'story' : 'stories'}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <div className="mt-4">
                <label className="cursor-pointer w-full">
                  ...
                  >
                    ...
                    {isUploading ? 'Uploading...' : book.coverUrl ? 'Change Cover' : 'Add Cover'}
                  </Button>
                  <input 
                    type="file" 
                    accept="image/*" 
            ...
            <div className="md:col-span-3">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-['Playfair_Display'] font-bold">Book Contents</h2>
              ...
              {bookMemories.length > 0 ? (
                <div className="space-y-4">
                  {bookMemories.map((memory, index) => (
                    <Card key={memory.id} className="bg-white border border-neutral-100 shadow-sm">
                      <CardContent className="p-4">
                        <div className="flex justify-between">
                          <div className="flex items-center space-x-4">
                            ...
                            <div>
                              <h3 className="font-bold text-neutral-900">{memory.title}</h3>
                              <p className="text-sm text-neutral-500 line-clamp-1">{memory.text}</p>
                            </div>
                          </div>
                          ...
                          {book.status !== 'complete' && (
                            <Button 
                              ...
                              onClick={() => handleRemoveMemoryFromBook(memory.id)}
                            >
                ...
                <Card className="bg-white border border-neutral-100 shadow-sm">
                  <CardContent className="p-6 text-center">
                    ...
                    <h3 className="font-medium text-neutral-700 mb-2">No stories added yet</h3>
          ...
          {availableMemories.length > 0 ? (
                ...
                {availableMemories.map((memory) => (
                  <div 
                    ...
                    onClick={() => {
                      if (selectedMemoryIds.includes(memory.id)) {
                        setSelectedMemoryIds(selectedMemoryIds.filter(id => id !== memory.id));
                      } else {
                  ...
                  >
                    <div className="flex items-start">
                      ...
                      <div>
                        <h3 className="font-medium text-neutral-900">{memory.title}</h3>
                        <p className="text-sm text-neutral-500 line-clamp-2 mt-1">{memory.text}</p>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="text-center py-6">
              <p className="text-neutral-500">
                You don't have any additional memories to add. Create more memories first!
              </p>
            </div>
          )}
          
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsAddMemoryDialogOpen(false)}>
              Cancel
            </Button>
            <Button 
            ...
            >
              Add {selectedMemoryIds.length} {selectedMemoryIds.length === 1 ? 'Memory' : 'Memories'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
